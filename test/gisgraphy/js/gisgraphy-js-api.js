...
(function(root) {
    ...
    function gisgraphyAutocomplete(o) {
        ...
        this.fulltextURL = 'http://services.gisgraphy.com/fulltext/suggest';
        this.reversegeocodingUrl = o.reversegeocodingUrl || '/reversegeocoding/search';
				//gpdc
        //this.geocodingUrl = o.geocodingUrl || '/geocoding/search';
        this.geocodingUrl = 'http://services.gisgraphy.com/geocoding/search';
...
/*
 ...
 Pattern 2 (ex: 40Â°26â€²47â€³N 079Â°58â€²36â€³W or 40Â°26'47"N 079Â°58'36"W)
 Pattern 3 (ex: 40d 26â€² 47â€³ N 079d 58â€² 36â€³ W or 40d 26' 47" N 079d 58' 36" W)
*/

function convertDMS(input) {
    var obj;
    var matches = input.match(/((\d+)\s?[:Â°dD]*\s?(\d+)\s?\s?[:'â€²]*\s?(\d+([.]\d+)?)?\s?[:\"â€³]*\s?([NnSs])\s?([,;]\s?)?(\d+)\s?[:Â°dD]*\s?(\d+)\s?[:'â€²]*\s?(\d+([.]\d+)?)?\s?[:\"â€³]*\s?([EeWw]))/);
    if (matches) {
...
function couldbeCoordinate(str) {
    match = str.match(/[-\d\sWwEeNnSsOo\.,:Â°'"]+/);
    if (match) {
        return match[0] == str;
    } else {
        return false;
...
convertDMS(50Â°37'59.7"N 3Â°03'13.2"E")
